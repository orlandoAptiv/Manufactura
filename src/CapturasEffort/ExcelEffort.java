...
public class ExcelEffort {
    ...
    String rutaArchivo = System.getProperty("user.home") + "/desktop/Reporte EFFORT_LM.xls";
    File archivoXLS;

    public ExcelEffort(DefaultTableModel modelo, ArrayList<Depto> Deptos) {
        ...
        try {
            ...
            hoja = libro.createSheet("EFFORT MOCHIS");
            ...
            for (int i = 0; i < Deptos.size(); i++) {
                for (int c = 1; c < modelo.getColumnCount(); c++) {
                    ...
                    celda = r.createCell(4);
                    celda.setCellValue(Double.parseDouble(modelo.getValueAt(3, c).toString()));
                }
                r = hoja.createRow(r.getRowNum() + 1);
                celda = r.createCell(1);
                celda.setCellValue(Deptos.get(i).iddepto);
...
//             Desktop.getDesktop().open(archivoXLS);
    ...
    public void gethojaScrapSq(DefaultTableModel modelo, ArrayList<Modulos> modulos, DefaultTableModel modeloPorc) {
        /*Se crea el objeto de tipo File con la ruta del archivo*/
        ...
        try {
            ...
            for (int i = 0; i < modeloPorc.getRowCount(); i++) {
                ...
                celda.setCellStyle(getTituloStyleTotales((int) HSSFColor.GREY_50_PERCENT.index));
                celda = r.createCell(5);
                celda.setCellValue(modeloPorc.getValueAt(i, 9).toString());
                celda.setCellStyle(getTituloStyleTotales((int) HSSFColor.GREY_50_PERCENT.index));
...
//             archivo.close();
    ...
    public void getPorcentajesHC(DefaultTableModel modeloPorcentajes, DefaultTableModel modeloHC) {
        /*Se crea el objeto de tipo File con la ruta del archivo*/
