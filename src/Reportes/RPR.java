...
public class RPR extends javax.swing.JFrame {
...
//         
    ...
    try{
        ...
        rs=Principal.cn.GetConsulta  ("select SUM(totaleshc) AS total_plata FROM (SELECT plataforma ,SUM(MANUFACTURA.HCDIRLINEA) AS HCLINEA, sum(manufactura.hcdirsoplps) as SopLPS, sum(manufactura.hcdirsoporte) as soporte, Sum(manufactura.HCDIRLPS) as LPS, SUM(manufactura.HCDIRCONTE) AS CONTENSION, SUM(manufactura.HCDIRFTQ) AS FTQ, SUM(manufactura.HCDIRPILOTOS) AS PILOTO, SUM(manufactura.hcdirsistemas) as sistemas, SUM(manufactura.HCDIRTABINSP) AS 'TAB.INSP', SUM(MANUFACTURA.HCRUTASINT) AS RUTASINTERNAS, SUM(c.hcdirecto) AS totaleshc FROM manufactura,personas as c WHERE manufactura.IDCODIGO = c.IDCODIGO AND MANUFACTURA.ACTIVO=1 and c.CADENA<>4 and c.cadena<>7   GROUP BY c.plataforma) AS totales");
        while(rs.next())
        {
       ...
       rs=Principal.cn.GetConsulta("SELECT plataforma ,SUM(MANUFACTURA.HCDIRLINEA) AS HCLINEA, sum(manufactura.hcdirsoplps) "+
               " as SopLPS, sum(manufactura.hcdirsoporte) as soporte, Sum(manufactura.HCDIRLPS) as LPS, SUM(manufactura.HCDIRCONTE) AS CONTENSION, SUM(manufactura.HCDIRFTQ) AS FTQ, SUM(manufactura.HCDIRPILOTOS) AS PILOTO, SUM(manufactura.hcdirsistemas) as sistemas, SUM(manufactura.HCDIRTABINSP) AS 'TAB.INSP', SUM(MANUFACTURA.HCRUTASINT) AS RUTASINTERNAS, "
               + "SUM(c.hcdirecto) AS totaleshc FROM manufactura, personas c  WHERE manufactura.IDCODIGO = c.IDCODIGO AND MANUFACTURA.ACTIVO=1 and c.CADENA<>4 and c.cadena<>7 and c.plataforma='"+gente.get(index).NombrePlataforma+"'  GROUP BY c.plataforma");
        if(rs.next())
        {
           totalhc=rs.getDouble("totaleshc");
              ...
              ModInge=Porciento*320;  // por que *202
       ...
       //query GAP 
        rs=Principal.cn.GetConsulta(" SELECT  c.IDCODIGO, c.CADENA,PLATAFORMA, c.CODIGO,  c.LINEA, c.TURNO,   \n" +
                                    ...
                                    "FROM codigos as c ,manufactura as m where m.IDCODIGO=c.IDCODIGO and c.cadena<>5 and c.cadena<>4 and c.cadena<>7  and m.ACTIVO=1 and c.LINEA<>98 and c.LINEA<>'29A'  GROUP BY c.PLATAFORMA");
         while(rs.next())
        {   
    ...
    public void CrearPaneles(){
        ...
        try {
         ...
         for(int i=0; i<gente.size(); i++){      
            ...
            Logger.getLogger(RPR.class.getName()).log(Level.SEVERE, null, ex);
        }
      ...
      {   
         ...
         this.getContentPane().add(CrearPanel(gente.get(i).NombrePlataforma,eficiencia,people, cuting, PorcCuting, SPL, PorcSPL, ENSFinal, PorcENSFinal, i+10));// por que + 10
         ...
         {  
           Object[] blanco=new Object[]{""," ","","" };
            ...
            Object[] gap=new Object[]{gente.get(i).soporte,gente.get(i).sop_lps,gente.get(i).tablero,gente.get(i).contension,gente.get(i).ftq,gente.get(i).kachirulesS,gente.get(i).sistemas,gente.get(i).pilotos,"",gente.get(i).rutas_int};
             this.getContentPane().add(panelGAP(gente.get(i).NombrePlataforma,blanco,gap,i+10));//+10 de que
    ...
    public void CrearPanelestotales(){
        ...
        try {
         ...
         totalcutMSD=(double) Math.round(totalcutMSD);
          ...
          Tpeoplecut_mfg=(totalcutmfg+totalSlp_mfg+totalF_assemblyMfg+kachirules2)+21; ///16 de que?
         ...
         Object[] ENSFinal=new Object[]{"Final Assembly", totalF_assemblyGsd,totalF_assemblyMsd,totalF_assemblyMfg+21,""};//sumo servicios
    ...
    public void  crearpanelTotalGAP(){
        ...
        try {
         ...
         Object[] peopleGAPtotal=new Object[]{soporte,TsopLPS,Ttablero,Tcontension,Tftq,kachirules2,Tsistemas,Tpilotos+21,"",Trutas };
    ...
    public JPanel panelGAP(String Nombre,Object[]blanco, Object[] peopleGAP, int locY){
        ...
        jpanel3.setLocation(460, valorAnteriorLoc-830);
    ...
    public void getGCSD(String Plataforma, int index){
        try {
            ResultSet rs =Principal.cn.GetConsulta("select c.PLATAFORMA, C.CODIGO, C.TURNO, sum(m.SALIDAENPIEZA) as 'SALIDA', sum(m.PUNTOSPZAPOND) as 'ptos.pza', SUM(g.HCDIRCORTE) as 'POND.CORTE', \n" +
                ...
                "FROM codigos as c, manufactura as m, gsd  as g where c.IDCODIGO=m.IDCODIGO and c.IDCODIGO=g.IDCODIGO AND  c.PLATAFORMA<>'CORTE' and  m.activo=1 and  c.plataforma='"+Plataforma+"' AND c.LINEA<>'91' AND c.LINEA<>'98' AND c.LINEA<>'29A' GROUP BY c.PLATAFORMA");
               if(rs.next())
               {
    ...
    public void getMSDyEnsFinal(String Plataforma, int index){
        try {
         ResultSet rs =Principal.cn.GetConsulta("SELECT\n" +
                    ...
                    "sum(m.HCDIRLPS) as 'LPSMSD', SUM(M.HCDIRLPS) AS 'LPS.MANUF', SUM(c.hcdirecto) AS 'ENS.FINALMANUF', \n" +
                    ...
                    "FROM personas as c , manufactura AS M  \n" +
                    "WHERE c.IDCODIGO =  M.IDCODIGO  and m.ACTIVO=1 and c.plataforma='"+Plataforma+"' AND c.LINEA<>'91' AND c.LINEA<>'98' AND c.LINEA<>'29A'  \n" +
                    "GROUP BY PLATAFORMA");
               if(rs.next())
               {
    ...
    public void inicializarArray(){
     try {
             ...
             ResultSet rs=Principal.cn.GetConsulta("SELECT DISTINCT\n" +
              "codigos.PLATAFORMA\n" +
              "FROM\n" +
              "codigos where codigos.cadena<>4 AND cODIGOS.LINEA<>'91' AND cODIGOS.LINEA<>'500' AND cODIGOS.LINEA<>'98' AND cODIGOS.LINEA<>'29A'  GROUP BY codigos.plataforma");
            while(rs.next())
            {
...
//           }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(RPR.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
