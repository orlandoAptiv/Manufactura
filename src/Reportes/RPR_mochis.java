       ...
       rs=Principal.cn.GetConsulta("SELECT plataforma ,SUM(MANUFACTURA.HCDIRLINEA) AS HCLINEA, sum(manufactura.hcdirsoplps) as SopLPS, sum(manufactura.hcdirsoporte) as soporte, Sum(manufactura.HCDIRLPS) as LPS, SUM(manufactura.HCDIRCONTE) AS CONTENSION, SUM(manufactura.HCDIRFTQ) AS FTQ, SUM(manufactura.HCDIRPILOTOS) AS PILOTO, SUM(manufactura.hcdirsistemas) as sistemas, SUM(manufactura.HCDIRTABINSP) AS 'TAB.INSP', SUM(MANUFACTURA.HCRUTASINT) AS RUTASINTERNAS, SUM(manufactura.HCDIRLINEA + manufactura.HCDIRSOPLPS + manufactura.HCDIRSOPORTE + manufactura.HCDIRLPS + manufactura.HCDIRCONTE + manufactura.HCDIRFTQ + manufactura.HCDIRPILOTOS + manufactura.HCDIRSISTEMAS + manufactura.HCDIRTABINSP + manufactura.HCRUTASINT) AS totaleshc FROM manufactura,codigos WHERE manufactura.IDCODIGO = codigos.IDCODIGO AND MANUFACTURA.ACTIVO=1 and codigos.CADENA<>4 and codigos.cadena<>7 and  codigos.cadena<>6 AND codigos.plataforma='"+gente.get(index).NombrePlataforma+"'  GROUP BY codigos.plataforma");
        if(rs.next())
        {
           totalhc=rs.getDouble("totaleshc");
              ...
              ModInge=Porciento*202;
              Porciento=Regresa2Decimales(Porciento);
              ModInge=Regresa2Decimales( ModInge);
...
" TRUNCATE(sum(HCDIRSOPORTE),0) AS 'SOPORTE', truncate(sum(HCDIRSOPLPS),0) AS 'SOP. LPS', TRUNCATE(sum(HCDIRTABINSP),0) AS 'TAB.INSP.', TRUNCATE(sum(hcdirconte),0) AS 'CONTENSION', TRUNCATE(sum(hcdirftq),0) AS 'FTQ', TRUNCATE(sum(hcdirsistemas),0) AS 'SISTEMAS', TRUNCATE(sum(hcdirpilotos),0) AS 'PILOTOS', TRUNCATE(sum(hcrutasint),0) AS 'RUTAS INT.'\n" +
    ...
    public void CrearPaneles(){
        ...
        try {
         ...
         for(int i=0; i<gente.size(); i++){      
            ...
            Logger.getLogger(RPR_mochis.class.getName()).log(Level.SEVERE, null, ex);
        }
      ...
      {   
         ...
         //cut_gcsd=EficienciaPlanta.Regresa2Decimales(cut_gcsd);
          ...
          cut_mfg=Regresa2Decimales(gente.get(i).HCCorteManuf);
          cut_mfg=Regresa2Decimales(cut_mfg);
          //cut_mfg=(double) Math.round(cut_mfg);
          
          slp_gsd=Regresa2Decimales(gente.get(i).HCLPSGCSD);
          slp_msd=Regresa2Decimales(gente.get(i).HCLPSMSD);
          slp_mfg=Regresa2Decimales(gente.get(i).HCLPSManuf);
          
          F_AssemblyGsd=Regresa2Decimales(gente.get(i).HCEnsFinalGCSD);
          F_AssemblyMsd=Regresa2Decimales(gente.get(i).HCEnsFinalMSD);
          F_AssemblyMfg=Regresa2Decimales(gente.get(i).HCEnsFinalManuf);
          /// total gente
          peoplecut_gsd=Regresa2Decimales(cut_gcsd+slp_gsd+F_AssemblyGsd);
         // peoplecut_gsd=(double) Math.round(peoplecut_gsd);
          peoplecut_msd=Regresa2Decimales( cut_msd+slp_msd+F_AssemblyMsd);
          
          peoplecut_mfg=Regresa2Decimales(cut_mfg+slp_mfg+F_AssemblyMfg+gente.get(i).kachirulesS);
          GAP=peoplecut_mfg-peoplecut_msd;
          GAP=(double) Math.round(GAP);
           efic_msd=Regresa2Decimales((peoplecut_gsd/peoplecut_msd)*100);
           efic_mfg=Regresa2Decimales((peoplecut_gsd/peoplecut_mfg)*100);                                
          /// GCSD  
         Object[] eficiencia=new Object[]{"Eficiencia",((Regresa2Decimales(peoplecut_gsd/peoplecut_gsd)*100)+"%"),efic_msd +"%",efic_mfg+"%","","" };
         Object[] people=new Object[]{"T. people",peoplecut_gsd,peoplecut_msd,peoplecut_mfg,GAP,"" };
         Object[] PorcCuting=new Object[]{"% CUTING",Regresa2Decimales((gente.get(i).HCCorteGCSD/gente.get(i).HCCorteGCSD)*100).toString()+" %",Regresa2Decimales((gente.get(i).HCCorteGCSD)/gente.get(i).HCCorteMSD*100).toString() +" %",Regresa2Decimales((gente.get(i).HCCorteGCSD/gente.get(i).HCCorteManuf)*100).toString()+" %",""};
         Object[] cuting=new Object[]{"CUTING",cut_gcsd, cut_msd, cut_mfg,""};
         Object[] PorcSPL=new Object[]{"% SLP",Regresa2Decimales((gente.get(i).HCLPSGCSD/gente.get(i).HCLPSGCSD)*100).toString()+ " %", Regresa2Decimales((gente.get(i).HCLPSGCSD)/gente.get(i).HCLPSMSD*100).toString()+" %", Regresa2Decimales((gente.get(i).HCLPSGCSD/gente.get(i).HCLPSManuf)*100).toString()+" %",""};
         Object[] SPL=new Object[]{"SLP",Math.round(gente.get(i).HCLPSGCSD), Regresa2Decimales(gente.get(i).HCLPSMSD).toString(), Regresa2Decimales(gente.get(i).HCLPSManuf).toString(),""};
         Object[] ENSFinal=new Object[]{"Final Assembly",(double) Math.round(gente.get(i).HCEnsFinalGCSD), Regresa2Decimales(gente.get(i).HCEnsFinalMSD).toString(),Regresa2Decimales(gente.get(i).HCEnsFinalManuf).toString(),""};
         Object[] PorcENSFinal=new Object[]{"% Final Assembly", Regresa2Decimales((gente.get(i).HCEnsFinalGCSD/gente.get(i).HCEnsFinalGCSD)*100).toString()+ " %",Regresa2Decimales((gente.get(i).HCEnsFinalGCSD)/gente.get(i).HCEnsFinalMSD*100).toString()+ " %", Regresa2Decimales((gente.get(i).HCEnsFinalGCSD)/gente.get(i).HCEnsFinalManuf*100).toString()+ " %",""};
        ...
        TCutporc_gsd=Regresa2Decimales((totalcutGSD/totalcutGSD)*100).toString() + "%";
        TCutporc_msd=Regresa2Decimales((totalcutGSD/totalcutMSD)*100).toString() + "%";
        TCutporc_mfg=Regresa2Decimales((totalcutGSD/totalcutmfg)*100).toString() + "%";
        
        Tslporc_gsd=Regresa2Decimales((totalSlp_gsd/totalSlp_gsd)*100).toString() + "%" ;
        Tslporc_msd=Regresa2Decimales((totalSlp_gsd/totalSlp_msd)*100).toString() + "%";
        Tslporc_mfg=Regresa2Decimales((totalSlp_gsd/totalSlp_mfg)*100).toString() + "%";
         
        Tassemblyporc_gsd=Regresa2Decimales((totalF_assemblyGsd/totalF_assemblyGsd)*100).toString() + "%";
               // Tassemblyporc_gsd=(double)Math.round((totalF_assemblyGsd/totalF_assemblyGsd)*100) + "%";

        Tassemblyporc_msd=Regresa2Decimales((totalF_assemblyGsd/totalF_assemblyMsd)*100).toString() + "%";
        Tassemblyporc_mfg=Regresa2Decimales((totalF_assemblyGsd/totalF_assemblyMfg)*100).toString() + "%";
    ...
    public  void crearpanelesGAP(){
       ...
       public void CrearPanelestotales(){
        ...
        try {
         ...
         totalcutmfg=Regresa2Decimales(totalcutmfg);
         totalcutmfg=(double) Math.round(totalcutmfg);
         
          totalcutMSD=Regresa2Decimales(totalcutMSD);
         totalcutMSD=(double) Math.round(totalcutMSD);
          ...
          Tpeoplecut_mfg=totalcutmfg+totalSlp_mfg+totalF_assemblyMfg+kachirules2+16;
          GAP_TOTAL=Tpeoplecut_mfg-Tpeoplecut_msd;
          GAP_TOTAL=(double) Math.round(GAP_TOTAL);
          
          
          Tefic_msd=Regresa2Decimales((Tpeoplecut_gsd/Tpeoplecut_msd)*100);
          Tefic_mfg=Regresa2Decimales((Tpeoplecut_gsd/Tpeoplecut_mfg)*100);   
          
       
         Object[] eficTotal=new Object[]{"Eficiencia",((Tpeoplecut_gsd/Tpeoplecut_gsd)*100+"%"),Tefic_msd+"%",Regresa2Decimales(Tefic_mfg)+"%","",""};
         ...
         Object[] ENSFinal=new Object[]{"Final Assembly", totalF_assemblyGsd,totalF_assemblyMsd,totalF_assemblyMfg+16,""};
     ...
     public void  crearpanelTotalGAP(){
        ...
        try {
         ...
         Object[] peopleGAPtotal=new Object[]{soporte,TsopLPS,Ttablero,Tcontension,Tftq,kachirules2,Tsistemas,Tpilotos+16,"",Trutas };
     ...
     public JPanel CrearPanel(String Nombre,Object[]eficiencia,Object[] people, Object[] cutting, Object[] PorcCuting,  Object[] lps, Object[] PorcLPS, Object[] ensamble, Object[] Porcensamble, int locY){
      ...
      public JPanel panelGAP(String Nombre,Object[]blanco, Object[] peopleGAP, int locY){
        ...
        jpanel3.setLocation(460, valorAnteriorLoc-590);
        jpanel3.setSize(450, 150);
        panelScroll3.setSize(500, 90);
    ...
    public void getGCSD(String Plataforma, int index){
        try {
            ResultSet rs =Principal.cn.GetConsulta("select c.PLATAFORMA, C.CODIGO, C.TURNO, sum(m.SALIDAENPIEZA) as 'SALIDA', sum(m.PUNTOSPZAPOND) as 'ptos.pza', SUM(g.HCDIRCORTE) as 'POND.CORTE', \n" +
                "truncate( SUM( if(c.TURNO='A', ((g.HCDIRCORTE*m.SALIDAENPIEZA)/9/100), ((g.HCDIRCORTE*m.SALIDAENPIEZA)/7.9/100))),0) aS 'GENTECORTE', \n" +
                "SUM(g.HCDIRLPS) as 'PONDLPS',  truncate (SUM( if(c.TURNO='A', ((g.HCDIRLPS *m.SALIDAENPIEZA)/9/100), ((g.HCDIRLPS*m.SALIDAENPIEZA)/7.9/100))),0) as 'GENTELPS',\n" +
                "SUM(g.HCDIRENSFINAL) as 'POND.ENSFINAL', \n" +
                "truncate(SUM( if(c.TURNO='A', ((g.HCDIRENSFINAL *m.SALIDAENPIEZA)/9/100), ((g.HCDIRENSFINAL*m.SALIDAENPIEZA)/7.9/100))),0) as 'GENTEENSFINAL'  \n" +
...
"  codigos.PLATAFORMA,codigos.CADENA,\n" +
"  IF(codigos.CADENA=3,TRUNCATE(sum(m.HCDIRLINEA),0),0) as 'LPSMSD', \n" +
"  IF(codigos.CADENA=3,TRUNCATE(SUM(M.HCDIRLINEA),0),0) AS 'LPS.MANUF',\n" +
"  IF(codigos.CADENA<>3, TRUNCATE(sum(m.HCDIRLINEA),0),0) as 'ENS.FINAL.MSD',\n" +
"  IF(codigos.CADENA<>3,TRUNCATE(SUM(HCDIRLINEA+HCDIRSOPLPS + hcdirconte+HCDIRSOPORTE+HCDIRTABINSP+hcrutasint+hcdirpilotos+hcdirftq+ hcdirsistemas),0),0) AS 'ENS.FINALMANUF',\n" +
"  IF(codigos.CADENA<>3,TRUNCATE(sum(m.HCDIRLINEA),0),0) as 'gentedirectamsd', \n" +
"  IF(codigos.CADENA<>3,TRUNCATE(SUM(M.HCDIRLPS+M.HCDIRSOPLPS +HCDIRLINEA+ hcdirconte+HCDIRSOPORTE+HCDIRTABINSP+hcrutasint+hcdirpilotos+hcdirftq+ hcdirsistemas),0),0) as 'gentedirectamanuf'\n" +
"   FROM codigos ,manufactura AS M  WHERE codigos.IDCODIGO =  M.IDCODIGO  and m.ACTIVO=1 and codigos.plataforma='"+Plataforma+"' AND codigos.LINEA<>'91' AND CODIGOS.CADENA<>6 AND CODIGOS.CADENA<>3  GROUP BY PLATAFORMA");
  
         ResultSet rs2 =Principal.cn.GetConsulta("SELECT\n" +
"  codigos.PLATAFORMA,codigos.CADENA,\n" +
"  IF(codigos.CADENA=3,TRUNCATE(sum(m.HCDIRLINEA),0),0) as 'LPSMSD', \n" +
"  IF(codigos.CADENA=3,TRUNCATE(SUM(M.HCDIRLINEA),0),0) AS 'LPS.MANUF',\n" +
"  IF(codigos.CADENA<>3, TRUNCATE(sum(m.HCDIRLINEA),0),0) as 'ENS.FINAL.MSD',\n" +
...
"  IF(codigos.CADENA<>3,TRUNCATE(sum(m.HCDIRLINEA),0),0) as 'gentedirectamsd', \n" +
"  IF(codigos.CADENA<>3,TRUNCATE(SUM(M.HCDIRLPS+M.HCDIRSOPLPS +HCDIRLINEA+ hcdirconte+HCDIRSOPORTE+HCDIRTABINSP+hcrutasint+hcdirpilotos+hcdirftq+ hcdirsistemas),0),0) as 'gentedirectamanuf'\n" +
"   FROM codigos ,manufactura AS M  WHERE codigos.IDCODIGO =  M.IDCODIGO  and m.ACTIVO=1 and codigos.plataforma='"+Plataforma+"' AND codigos.LINEA<>'91' AND CODIGOS.CADENA<>6 AND CODIGOS.CADENA=3  GROUP BY PLATAFORMA,CADENA");
         ...
         if(rs.next())
           ...
           if(rs2.next())
               {
                   gente.get(index).HCLPSMSD=rs2.getDouble("LPSMSD");
                   gente.get(index).HCLPSManuf=rs2.getDouble("LPS.MANUF");
    ...
    public void inicializarArray(){
     try {
             ...
             ResultSet rs=Principal.cn.GetConsulta("SELECT DISTINCT codigos.PLATAFORMA, codigos.CADENA, codigos.CODIGO,codigos.ACTIVO FROM codigos INNER JOIN manufactura on codigos.IDCODIGO=manufactura.IDCODIGO\n" +
"           where codigos.cadena<>4 and codigos.CADENA<>6 and codigos.CADENA<>0  AND cODIGOS.LINEA<>'91' AND manufactura.ACTIVO=1  GROUP BY codigos.plataforma");
            while(rs.next())
            {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
       ...
       try{
//           for(PlataformaGenteCorte g:gente){
               ExcelRPR exportar=new ExcelRPR(gente,  "RPR", (DefaultTableModel) tabla2.getModel(),gente, gente.get(0).totalGap);
//           }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(RPR_mochis.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new RPR_mochis().setVisible(true);
            }
        });
    }
